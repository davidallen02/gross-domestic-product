...
reported <- pamngr::get_data("gdpctot%") %>%
  magrittr::set_colnames(c("dates", "value")) %>%
  dplyr::mutate(variable = "Reported")

estimates <- pamngr::join_sheets(c("echhus q320", 
                                   "echhus q420", 
                                   "echhus q121",
                                   "echhus q221")) %>%
...
reported %>%
  ...
  pamngr::barplot() %>%
  pamngr::pam_plot(
    plot_title = "US Consumer Spending",
    plot_subtitle = "QoQ SAAR",
    caption = FALSE
  ) %>%
  pamngr::all_output("consumption-growth-with-estimates")
